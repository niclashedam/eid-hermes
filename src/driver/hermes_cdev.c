...
static int __cmd_comp(struct hermes_dev *hermes, int eng)
{
	...
	ctrl.ehcmdctrl = ioread8(&hermes->cmds_ctrl[eng].ehcmdctrl);

	return ctrl.ehcmddone;
}
...
static int hermes_fsync(struct file *filp, loff_t start, loff_t end, int datasync)
{
	...
	struct hermes_cmd cmd = {
		.req = {
			...
			.cid = env->cid,
			.run_prog.prog_slot = env->prog_slot,
			.run_prog.data_slot = env->data_slot,
		},
	};
	int eng = 0, res;
	int64_t ebpf_ret;
	...
	pr_debug("opcode: 0x%x cid: 0x%x prog_slot: 0x%x data_slot: 0x%x\n",
			...
			cmd.req.run_prog.data_slot);

	memcpy_toio(&hermes->cmds[eng].req, &cmd.req, sizeof(cmd.req));
	iowrite8(1, &hermes->cmds_ctrl[eng].ehcmdctrl);

	res = wait_event_interruptible(hermes->wq[eng], __cmd_comp(hermes, eng));
	if (res)
		goto out;
	...
	case HERMES_SUCCESS:
		ebpf_ret = hermes->cmds[eng].res.run_prog.ebpf_ret;
		if (ebpf_ret) {
			dev_warn(&hermes->dev,
...
static ssize_t hermes_read_write_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	if (write && iocb->ki_flags & IOCB_SYNC) {
...
static int hermes_read_cfg(struct hermes_pci_dev *hpdev)
{
	...
	if (cfg->eheng > EBPF_ENG_NUM_MAX) {
		dev_warn(&hpdev->hdev->dev,
			 "Number of eBPF engines (%d) exceeds maximum. Decreasing to %d.\n",
...
{
	void __iomem *bar0 = pci_iomap(hpdev->pdev, 0,
				HERMES_CMDCTRL_BASE + hpdev->hdev->cfg.eheng);
	if (!bar0)
		return -EFAULT;
